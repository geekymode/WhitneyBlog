...
```{r figSunriseWeb, fig.margin = TRUE, fig.width= 8, fig.height= 8, fig.cap = "An image sourced from [Brad Spiess](https://www.ihikesandiego.com/about-me/)'s blog. I had always wished to witness and perhaps even capture an image such as this one by [Brad Spiess](https://www.ihikesandiego.com/about-me/). You can see this image [The whitney sunrise from the trail camp near the iceberg lake](https://www.ihikesandiego.com/wp-content/uploads/2013/10/Sunrise-at-Trail-Camp-small.jpg). His fantastic blog is worth a read. We were touch late to get to the trail camp to witness this experience.", message=TRUE, cache=TRUE, echo=FALSE, out.width='100%'}
...
After nearly six hours of steady climbing through the pre dawn darkness, we reached the trail camp at 7 AM. The two hours leading upto was a unique spectacle staged on a vast arena of the mountain passage. The eastern sky was beginning to blush with the first hints of sunrise, gradually revealing the magnificent landscape that had been hidden in darkness during our ascent. I will never forget that experience.
...
We spent two hours at trail camp, partly to rest and refuel, but mostly to soak in the incredible scenery that daylight had unveiled. The granite peaks surrounding us caught the early morning light, transforming from shadowy silhouettes into gleaming monuments of stone. Waterfalls cascaded down distant cliffs, golden lights flashed for a few seconds, but once arrived on the stage, sun was srong and intense. Did the mountain creeks sing their eternal songs as snow melt found its way downhill? Perhaps!


```{r figFullWhitney, fig.fullwidth=TRUE,fig.margin = FALSE, fig.cap = "The sun kissed the Whitney peaks, and she blushed rose for a second. Elle s'est teintée de rose pendant un instant!", message=TRUE, cache=TRUE, echo=FALSE, out.width='100%'}
...
At this point, we faced two challenging options. The first was the regular trail, which ascends through 99 switchbacks. While this route is normally straightforward but tiring, the ice that day made it treacherous. Every step required careful consideration due to frozen patches scattered along the path, creating far from ideal conditions.
...
```{r figWoodPath, fig.margin = FALSE, fig.width= 8, fig.height= 8, fig.cap = "Seeing the few wooden crossings on the trail, and after witnessing many streams, mountains and timber, I was thinking about Neruda's words: *'I grew up in this town, my poetry was born between the hill and the river, it took its voice from the rain, and like the timber, it steeped itself in the forests.'* Neruda was born in 1904 in the mountainous frontier region of Temuco in Chile. I have never been there, but the material makeup of this landscape made me draw some remote parallels.", message=TRUE, cache=TRUE, echo=FALSE, out.width='100%'}
...
```{r figCable, fig.margin = TRUE, fig.width= 8, fig.height= 8, fig.cap = "The cable section on the 99 switch back route had quite of snow to be dealt with. It wass at this juncture, our first team decided to call off plans to ascend any farther.", message=TRUE, cache=TRUE, echo=FALSE, out.width='100%'}
...
The second option was the hard chute, a more direct but significantly more technical route to the summit. This dangerous path required climbing straight up using ice axes and was covered in ice. The chute represents the classic mountaineering challenge that Mt. Whitney offers, demanding both physical endurance and technical skill.

With conditions poor on both routes, our group split up. The majority of our team attempted the 99 switchback trail, while four of us decided to tackle the chute despite its technical difficulties.

The first team went all the way to the cable section and decided to return after assessing the snow situation. The other team (including yours truly) made it halfway up the chute before wisdom overcame ambition. The ice conditions, which had been merely sketchy lower down, were becoming genuinely dangerous. As the morning sun began to warm the slopes, the ice started turning slushy, and our crampons (adequate for the hard conditions we'd encountered earlier) were no longer providing the reliable grip we needed for safe passage. Once we realized that we wouldnt be able to summit before noon, we made the difficult decision to stop ascending further. 
...
```{r figBlueBird, fig.margin = TRUE, fig.width= 8, fig.height= 8, fig.cap = "Plenty of Steller's Jay birds showing of their beauty all around. This one was quite friendly.", message=TRUE, cache=TRUE, echo=FALSE, out.width='100%'}
knitr::include_graphics("images/bird.jpg")
```



```{r figFlower, fig.margin = FALSE, fig.width= 8, fig.height= 8, fig.cap = "വിടർന്നൂ മരുഭൂവിൻ എരിവെയിലിലും പൂക്കൾ, Flowers bloomed even in the scorching sun of the desert. Even in this heat, there was music in the air, there were the occasional flowers and ofcourse plenty of hope. The Malayalam script is from this famous song from [Moonampakkam](https://www.youtube.com/watch?v=KX7dikJQA4w). We spotted some alpine shooting stars ([primula tetrandra](https://en.wikipedia.org/wiki/Primula_tetrandra)) in the meadows. ", message=TRUE, cache=TRUE, echo=FALSE, out.width='100%'}
knitr::include_graphics("images/flower.jpg")
```


```{r figBlueBird2, fig.margin = TRUE, fig.width= 8, fig.height= 8, fig.cap = "She turns and insists on this pose. The blue spots and black shades look better this way? I happily oblige and clicked one!", message=TRUE, cache=TRUE, echo=FALSE, out.width='100%'}
...
```{r figBlueBirdX, fig.margin = TRUE, fig.width= 8, fig.height= 8, fig.cap = "May be her partner watching from here not approving this of?!", message=TRUE, cache=TRUE, echo=FALSE, out.width='100%'}
knitr::include_graphics("images/birdX.jpg")
```
...
This  is mid June and technically beginning of the summer here in California. It looked as though spring was still asserting itself in some areas while winter held firm in others, creating a dynamic landscape that seemed to change with every turn of the trail.
...
```{r figGateend, fig.margin = TRUE, fig.width= 8, fig.height= 8, fig.cap = "Trail gate finally. We started the ascend through this pretty gate in the dark and returned through the very same gate at dusk. In between was the experience of a lifetime that we gathered.", message=TRUE, cache=TRUE, echo=FALSE, out.width='100%'}
...
The article itself is written in R Markdown. All photos (except one which is explicitly mentioned and source inclued) are taken by myself or my hiking friends, **The Trail Blazers**. For typesetting, I have used the Tufte handout style format (thanks to [Edward Tufte](https://en.wikipedia.org/wiki/Edward_Tufte)), which reads best when viewed on a computer or, if on a phone/tablet, when the device is held horizontally.
